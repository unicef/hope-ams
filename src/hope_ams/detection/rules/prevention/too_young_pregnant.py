...
class TooYoungPregnantConfig(RuleConfigForm):
    min_age = forms.IntegerField(label="Minimum age", initial=15)


class TooYoungPregnantRule(BaseRule):
    name = "too_young_pregnant"
    verbose_name = "Too Young Pregnant"
    phase = "prevention"
    config_class = TooYoungPregnantConfig
    description = "Individual is marked as pregnant but age is below expected minimum"
    default_severity = "critical"
    default_config = {"min_age": 15}

    def evaluate(self, ctx: RuleContext) -> list[Finding]:
        findings = []
        today = datetime.now(tz=UTC).date()
        min_age = self.get_config(ctx).get("min_age", 15)
