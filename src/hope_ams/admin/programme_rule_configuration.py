...
@admin.register(ProgrammeRuleConfiguration)
class ProgrammeRuleConfigurationAdmin(ExtraButtonsMixin, UnfoldModelAdmin):  # type: ignore[misc]
    list_display = [
        "programme",
        "name",
        "display_rule",
    ...
    def display_rule(self, obj: ProgrammeRuleConfiguration) -> str:
    ...
    def configure(self, request: HttpRequest, pk: str) -> HttpResponse:
        obj: ProgrammeRuleConfiguration | None = self.get_object(request, pk)
        if obj is None:
            raise Http404
        ...
        if form_class:
            if request.method == "POST":
                ...
                if config_form.is_valid():
                    ...
                    self.message_user(
                        request,
                        _("Rule %(name)s configured for %(programme)s")
                        % {"name": obj.name, "programme": obj.programme},
                    )
                    return redirect(
                        reverse(
